...
#================================================
#   Calculo de curva Z-spline en ndimensiones
#================================================
...
class Curva:
    ...
    """=======================================================
    Cuva construye la curva que pasa por los puntos x
    ==========================================================
    Parametros: x coordenadas ordenadas ((x1), (x2),......)
                f propiedades (f1(c),f2(x),....)
                dim dimensiones
    """
    #================
    #  Constructor
    ...
    def longitud(s) -> None:
        ...
        for i in range(0,s.n):
            ...
            if i == s.n-1:
                ip1 = 0
            d:np.float64 = (s.x[ipl]-s.x[i])**2
            for j in range(1,s.dim):
                d += (s.x[ipl+j*s.n]-s.x[i+j*s.n])**2
            t += d**0.5
            s.l.append(t)
        s.L = t
        s.dx = t/float(s.n)
    ...
    def interpolacion(s,p: int=0,r:float=0.0)  -> list:
        ...
        if p == 0:
            ...
            if i == s.n-1:
                ipl = 0
            xi.append(a*s.x[ipl] + (1-0-a)*s.x[i])
            for j in range(1,s.dim):
                xi.append(a*s.x[iṕ1+j*s.n]+(1.0-a)*s.x[i+j*s.n])
        ...
        #===============================
          ...
          zp2:np.float64 = 0.5*(2.0-ap2)*(1.0-ap2)
     ...
     #=====================================
           ...
           zm1:np.float64 = -4.0+u12*am1*(225.0+am1*(-367.5+am2*(-156.5+am2*(31.5-2.5*am2))))
...
#======================================================================================
#   Funcion zpline crea el Z-spline de un conjunto de puntos en dim dimensiones
#   n segmentos y continuidad (aproximacion) cont
#   x,y = zpline(conjunto de puntos, dimension, numero de segmentoss, continuidad)
